...
public class ReportPayOutAdaptor  {
	public List<ReportPayOutDTO> modelListtoDTOList(List<Object[]> reportPayOutList){
		...
		for (Object[] reportPayOutObj : reportPayOutList) {
			...
			BigDecimal NetPayableAmount=reportPayOutObj[1]!=null?(BigDecimal)reportPayOutObj[1]:null;
...
public ReportPayOutDTO databaseModelToUiDto(ReportPayOut reportPayOut) {
	...
	//System.out.println("process Month------------------------------------------------------------------"+reportPayOut.getProcessMonth());
	
	ReportPayOutDTO reportPayOutDto=new ReportPayOutDTO();
	
	reportPayOutDto.setPayDays(reportPayOut.getPayDays());
	...
	reportPayOutDto.setProcessMonth(reportPayOut.getId().getProcessMonth());
	 ...
	 for (Object[] reportPayOutObj : reportPayOutListObj) {
		...
		Date dateOfJoining=reportPayOutObj[3]!=null?(Date)reportPayOutObj[3]:null;
 		...
 		Long employeeId=reportPayOutObj[7]!=null?Long.parseLong(reportPayOutObj[7].toString()):null;
  		...
  		String gradesName=reportPayOutObj[9]!=null?(String)reportPayOutObj[9]:null;
  		...
  		reportPayOutSalaryDto.setGradesName(gradesName);
...
/*SELECT rp.Name, rp.employeeCode,rp.bankName ,rp.accountNumber,rp.IFSCCode,rp.NetPayableAmount ,dept.departmentName\r\n" + 
"	 FROM ReportPayOut  rp JOIN Department dept ON dept.departmentId = rp.departmentId  where   rp.processMonth=?2 and rp.companyId =?1
*/public List<ReportPayOutDTO> reportPayoutObjToRepoortPayDTO(List<Object[]>  reportPayOutListObj) {
	 List<ReportPayOutDTO> reportPayOutDtoList=new ArrayList<ReportPayOutDTO>();
	 for (Object[] reportPayOutObj : reportPayOutListObj) {
		 ReportPayOutDTO reportPayOutDto = new ReportPayOutDTO();
		 String name=reportPayOutObj[0]!=null?(String)reportPayOutObj[0]:null;
		 ...
		 String IFSCCode=reportPayOutObj[4]!=null?(String)reportPayOutObj[4]:null;
		 ...
		 reportPayOutDto.setEmployeeCode(employeeCode);
		 reportPayOutDto.setBankName(BankName);
		 reportPayOutDto.setAccountNumber(accountNo.toString());
		 reportPayOutDto.setIfscCode(IFSCCode);
		 reportPayOutDto.setNetPayableAmount(netPayableAmount);
		 reportPayOutDto.setEmployeeId(employeeId);
...
public List<ReportPayOutDTO> dataBaseModelObjtoUiDto(List<Object[]> salaryPendingList)
{
	List<ReportPayOutDTO> reportPayOutDtoList=new ArrayList<ReportPayOutDTO>();
	for(Object[] reportPayOutObj:salaryPendingList)
	{
		ReportPayOutDTO reportPayOutDTO=new ReportPayOutDTO();
		String processMonth=reportPayOutObj[0]!=null?(String)reportPayOutObj[0]:null;
		BigDecimal netPayableAmount=reportPayOutObj[1]!=null?(BigDecimal)reportPayOutObj[1]:null;
		BigDecimal netPayableAmountTotal=reportPayOutObj[2]!=null?(BigDecimal)reportPayOutObj[2]:BigDecimal.ZERO;
		BigDecimal payDays=reportPayOutObj[3]!=null?(BigDecimal)reportPayOutObj[3]:null;
		System.out.println("************************************************************************************************************"+netPayableAmountTotal);
		reportPayOutDTO.setProcessMonth(processMonth);
		reportPayOutDTO.setNetPayableAmount(netPayableAmount);
		reportPayOutDTO.setNetPayableAmountTotal(netPayableAmountTotal);
		reportPayOutDTO.setPayDays(payDays);
		reportPayOutDtoList.add(reportPayOutDTO);
...
public ReportPayOutDTO lastPaodModelToLastPaidDTO(List<Object[]> lastPaid) {
	ReportPayOutDTO reportPayOutDTO=new ReportPayOutDTO();
...
public List<InvestmentFinancialYearDTO> investmentDatabaseModelToUiDtoLists(List<Object[]> objectList) {
	// DesignationDTO designationDto = new DesignationDTO();
	List<InvestmentFinancialYearDTO> investmentDtoList = new ArrayList<InvestmentFinancialYearDTO>();
	String activeStatus = null;
	for (Object[] obj : objectList) {
	InvestmentFinancialYearDTO dto = new InvestmentFinancialYearDTO();
	if (obj[0] != null) {
	dto.setFinancialYear(obj[0].toString());
	activeStatus = obj[0].toString();
	}
	investmentDtoList.add(dto);
	}
