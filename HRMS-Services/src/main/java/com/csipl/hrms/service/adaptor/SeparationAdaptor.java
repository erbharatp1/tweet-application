...
public class SeparationAdaptor implements Adaptor<SeparationDTO, Separation> {
	...
	public Separation uiDtoToDatabaseModel(SeparationDTO separationDto) {
		...
		separation.setExitDate(separationDto.getExitDate());
	...
	//			.getDropDownValue(DropDownEnum.ResignationReason.getDropDownName(), separation.getResoan()));
		System.out.println("ID "+separationDto.getApprovalId());
 		...
 		separationDto.setDateUpdate(separation.getDateUpdate());
		separationDto.setExitDate(separation.getExitDate());
	...
	public List<SeparationDTO> modelListToDtoList(List<Object[]> separationList, EmployeeSearchDTO employeeSearcDto) {
		...
		for (Object[] separationObj : separationList) {
			...
			if (separationObj[12] != null) {
				separationDto.setExitDate((Date)separationObj[12]);
			}
			if (separationObj[13] != null) {
				separationDto.setEmployeeLogoPath(separationObj[13].toString());
			}
			if (separationObj[14] != null) {
				separationDto.setEmployeeCode(separationObj[14].toString());
			}
	...
	public List<PositionDTO> modelListToDtoList(List<Object[]> separationList) {
		List<PositionDTO> positionDTOList = new ArrayList<PositionDTO>();
		for (Object[] separationObj : separationList) {
			PositionDTO positionDTO = new PositionDTO();
			String employeeName = separationObj[0] != null ? separationObj[0].toString() : null;
			Long employeeId = separationObj[1] != null ? Long.parseLong(separationObj[1].toString()) : null;
			String employeeCode = separationObj[2] != null ? separationObj[2].toString() : null;
			String departmentName = separationObj[3] != null ? separationObj[3].toString() : null;
			String designationName = separationObj[4] != null ? separationObj[4].toString() : null;
			String grades = separationObj[5] != null ? separationObj[5].toString() : null;
			String empType = separationObj[6] != null ? separationObj[6].toString() : null;
			Date endDate = separationObj[7] != null ?    (Date)separationObj[7] : null;
			Long gradesId = separationObj[8] != null ? Long.parseLong(separationObj[8].toString()) : null;
			String jobLocation = separationObj[9] != null ? separationObj[9].toString() : null;
			positionDTO.setEmployeeName(employeeName);
			positionDTO.setEmployeeId(employeeId);
			positionDTO.setEmployeeCode(employeeCode);
			positionDTO.setDepartmentName(departmentName);
			positionDTO.setDesignationName(designationName);
			positionDTO.setGradeName(grades);
			positionDTO.setEmpType(empType);
			positionDTO.setEndDate(endDate);
			positionDTO.setGradeId(gradesId);
