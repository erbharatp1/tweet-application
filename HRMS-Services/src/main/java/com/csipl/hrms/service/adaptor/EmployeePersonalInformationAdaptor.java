...
public class EmployeePersonalInformationAdaptor {
	...
	public Employee uiDtoToDatabaseModel(EmployeeDTO employeeDto) {
		...
		employee.setTdsLockUnlockStatus(StatusMessage.TDS_LOCKED_STATUS);
		return employee;
	}
	...
	public EmployeeDTO databaseModelToUiDto(Employee employee) {
		...
		if (employee != null && employee.getDateOfJoining() != null && employee.getEndDate() != null)
			employeeDto.setEmploymentWithUs(
					(DateUtils.getDateDifferenceInDDMMYYYY(employee.getDateOfJoining(), employee.getEndDate())));
		else
			employeeDto.setEmploymentWithUs(
					(DateUtils.getDateDifferenceInDDMMYYYY(employee.getDateOfJoining(), new Date())));
		...
		if (employee.getBranch() != null) {
			employeeDto.setBranchId(employee.getBranch().getBranchId());
		}
		...
		employeeDto.setFullNameCodeVaues(
				employee.getFirstName() + " " + employee.getLastName() + " (" + employee.getEmployeeCode() + ")");
		employeeDto.setGradesId(employee.getGradesId());
		employeeDto.setContactNo(employee.getContactNo());
		employeeDto.setCharFirstName(Character.toString(employee.getFirstName().charAt(0)).toUpperCase());
		employeeDto.setCharLastName(Character.toString(employee.getLastName().charAt(0)).toUpperCase());
		employeeDto.setTdsPlanType(employee.getTdsPlanType());
...
//		employeeDto.setWorkingMonths(diffMonth);
	...
	public List<EmployeeDTO> databaseModelToUiDtoListNativeSearch(List<Employee> employeeList) {
		...
		for (Employee employee : employeeList) {
			...
			dto.setBiometricId(employee.getBiometricId());
	...
	public EmployeeDTO databaseModelToUiDtoCheckInList(Employee employeeDetails) {

		EmployeeDTO dto = new EmployeeDTO();

		dto.setEmployeeId(employeeDetails.getEmployeeId());
		dto.setEmployeeCode(employeeDetails.getEmployeeCode());
		dto.setFirstName(employeeDetails.getFirstName());
		dto.setLastName(employeeDetails.getLastName());
		dto.setBiometricId(employeeDetails.getBiometricId());
		dto.setDateOfJoining(employeeDetails.getDateOfJoining());
	...
	public List<EmployeeDTO> modeltoDTOList(List<Object[]> employeeList, EmployeeSearchDTO employeeSearchDto) {
		...
		for (Object[] empObj : employeeList) {
			...
			if (empObj[13] != null) {
				Date resignationDate = (Date) (empObj[13]);
				emp.setResignationDate(resignationDate);
			}
...
//		}
	...
	public List<EmployeeDTO> databaseObjectModelToUiDto(List<Object[]> employeeObjList) {
	...
	public List<EmployeeDTO> databaseObjModelToUiDtoListEmployee(List<Object[]> employeeObjList) {
		...
		for (Object[] employeeObj : employeeObjList) {
			...
			if (employeeObj[4] != null) {
				employeeDto.setDepartmentName(employeeObj[4].toString());
			}

			employeeDto.setFullNameCodeVaues(employeeDto.getFirstName() + " " + employeeDto.getLastName() + " ("
					+ employeeDto.getEmployeeCode() + ")" + " " + employeeDto.getDepartmentName());
	...
	public List<EmployeeDTO> databaseObjModelToUiDtoListNotifyEmployee(List<Object[]> employeeObjList) {
		...
		for (Object[] employeeObj : employeeObjList) {
			...
			if (employeeObj[5] != null) {
				employeeDto.setEmailId(employeeObj[5].toString());
			}
	...
	/**
	 * 
	 * databaseModelToUiDtoEmpBdayList
	 */
	public List<EmployeeReportDTO> databaseModelToUiDtoEmpBdayList(List<Object[]> employeeReportList) {

		List<EmployeeReportDTO> empReportDTOList = new ArrayList<EmployeeReportDTO>();

		for (Object[] employeeReport : employeeReportList) {
			EmployeeReportDTO employeeReportDTO = new EmployeeReportDTO();
			if (employeeReport[0] != null) {
				employeeReportDTO.setEmpName(employeeReport[0].toString());
			}

			if (employeeReport[1] != null) {
				employeeReportDTO.setEmpCode(employeeReport[1].toString());
			}
			if (employeeReport[2] != null) {
				employeeReportDTO.setDepartmentName(employeeReport[2].toString());
			}
			if (employeeReport[3] != null) {
				employeeReportDTO.setDesignationName(employeeReport[3].toString());
			}
			if (employeeReport[4] != null) {
				employeeReportDTO.setBirthdaydate(employeeReport[4].toString());
			}
			if (employeeReport[5] != null) {
				employeeReportDTO.setEmployeeLogoPath(employeeReport[5].toString());
			}
			empReportDTOList.add(employeeReportDTO);
		}

		return empReportDTOList;
	}

	/**
	 * databaseModelToUiDtoEmpAnniversaryList}
	 */
	public List<EmployeeReportDTO> databaseModelToUiDtoEmpWorkAnniversaryList(List<Object[]> list) {
		List<EmployeeReportDTO> empReportDTOList = new ArrayList<EmployeeReportDTO>();
		for (Object[] employeeReport : list) {
			EmployeeReportDTO employeeReportDTO = new EmployeeReportDTO();
			if (employeeReport[0] != null) {
				employeeReportDTO.setEmpName(employeeReport[0].toString());
			}
			...
			if (employeeReport[4] != null) {
				employeeReportDTO.setJoiningDate(employeeReport[4].toString());
			}
			if (employeeReport[5] != null) {
				employeeReportDTO.setEmployeeLogoPath(employeeReport[5].toString());
			}
	...
	public List<EmployeeReportDTO> databaseModelToUiDtoEmpAnniversaryList(List<Object[]> list) {
		List<EmployeeReportDTO> empReportDTOList = new ArrayList<EmployeeReportDTO>();
		...
		for (Object[] employeeReport : list) {
			...
			if (employeeReport[4] != null) {
				employeeReportDTO.setAnniversarydate(employeeReport[4].toString());
			}
			if (employeeReport[5] != null) {
				employeeReportDTO.setEmployeeLogoPath(employeeReport[5].toString());
			}
	...
	public List<EmployeeDTO> databaseObjModelToUiEmployeeLifeCycleDto(List<Object[]> employeeObjList) {
		...
		for (Object[] employeeObj : employeeObjList) {
			...
			if (employeeObj[1] != null) {
				employeeDto.setFirstName(employeeObj[1] != null ? employeeObj[0].toString() : null);
			}

			if (employeeObj[2] != null) {
				employeeDto.setDesignationName(employeeObj[2] != null ? employeeObj[2].toString() : null);
			}
			if (employeeObj[3] != null) {
				employeeDto.setDepartmentName(employeeObj[3] != null ? employeeObj[3].toString() : null);
			}
			if (employeeObj[4] != null) {
				employeeDto.setDateUpdated(employeeObj[4] != null ? (Date) employeeObj[4] : null);

				Date date = employeeObj[4] != null ? (Date) employeeObj[4] : null;
				String[] ymd = date.toString().split("-");
				int year = Integer.parseInt(ymd[0]);
				employeeDto.setYear(year);
			}
			employeeDtoList.add(employeeDto);
		}
		return employeeDtoList;
	...
	public List<EmployeeDTO> databaseObjModelToUiRollbackDtoList(List<Object[]> employeeObjList) {
		...
		for (Object[] employeeObj : employeeObjList) {
			...
			employeeDto.setDateCreated((Date) employeeObj[9] != null ? (Date) employeeObj[9] : null);
	...
	public List<EmployeeDTO> databaseModelToUiDtoListEmployee(List<Employee> employeeList) {
		List<EmployeeDTO> employeeDtoList = new ArrayList<EmployeeDTO>();
		for (Employee employee : employeeList) {
			employeeDtoList.add(databaseModelToUiDtoEmployee(employee));
		}
		return employeeDtoList;
	}

	private EmployeeDTO databaseModelToUiDtoEmployee(Employee employee) {
		EmployeeDTO employeeDto = new EmployeeDTO();
		employeeDto.setEmployeeId(employee.getEmployeeId());
		employeeDto.setEmployeeCode(employee.getEmployeeCode());
		employeeDto.setFirstName(employee.getFirstName());
...
//	public List<EmployeeReportDTO> databaseModelToUiDtoMyTeamLeaveList(List<EmployeeReportDTO[]> list) {
//		List<EmployeeReportDTO> empReportDTOList = new ArrayList<EmployeeReportDTO>();
//		for (EmployeeReportDTO[] employeeReport : list) {
//			EmployeeReportDTO employeeReportDTO = new EmployeeReportDTO();
//			if (employeeReport[0] != null) {
//				employeeReportDTO.setEmpName(employeeReport[0].toString());
//			}
