...
public interface CandidateRepository extends CrudRepository<Candidate, Long> {
	...
	String findWPatternName = "SELECT  pattern.patternName,pattern.patternId FROM TMSWeekOffMasterPattern pattern WHERE pattern.patternId=?1";
	...
	@Modifying
	@Query("UPDATE Candidate SET candidateStatus=?1 WHERE candidateId=?2")
	void changeStatus(String candidateStatus, Long candidateId);
