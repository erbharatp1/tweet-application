...
public class CandidateToEmployeeConversionAdaptor {
	...
	public Employee candidateInfoToEmployeeInfoConversion(Candidate candidate,
		...
		if(candidate.getReportingToEmployee()!=null)
		...
		employee.setBiometricId(candidateOfficialInfo.getBiometricId());
		if(candidateOfficialInfo.getEmployeeCode() != null) {
			employee.setEmployeeCode(candidateOfficialInfo.getEmployeeCode());
			
		}
		employee.setEmployeeCodeStatus(candidateOfficialInfo.getEmployeeCodeStatus());
			//System.out.println("candidatePersonal.getDateOfBirth().."+candidatePersonal.getDateOfBirth());
		...
		employee.setAdharNumber(candidate.getAdharNumber());
		employee.setShiftId(candidate.getShiftId());
		employee.setPatternId(candidate.getPatternId());
		employee.setPersonalEmail(candidate.getEmailId());
		employee.setOfficialEmail(candidateOfficialInfo.getOfficialEmail());
		employee.setContactNo(candidate.getMobile());
	    employee.setLeaveSchemeId(candidate.getLeaveSchemeId());
	    employee.setAttendanceSchemeId(candidate.getAttendanceSchemeId());
		...
		if(candidate.getDateOfJoining()!=null)
		employee.setEmpType(StatusMessage.EMPLOYEE_TYPE_PERMANENT);
		else
			employee.setEmpType(StatusMessage.EMPLOYEE_TYPE_CONTRACT);
		...
		if(candidate.getBranchId()!=null) {
			branch.setBranchId(candidate.getBranchId());
...
public List<EmployeeLanguage> languageDtoListToDatabaseModelList(List<CandidateLanguage> candidateLanguageList,Employee employee) {
	...
	public List<EmployeeIdProof> candidateIdProofToEmployeeIdProofConversion(
		...
		for (CandidateIdProof candidateIdProof : candidateIdProofList) {
			...
			employeeIdProof.setIdProofDoc(candidateIdProof.getIdProofDoc());
	...
	public List<ProfessionalInformation> candidateProfessionalInfoToEmployeeProfessionalInfoConversion(
		...
		for (CandidateProfessionalInformation candidateProfessionalInformation : candidateProfessionalInformationList) {
			...
			professionalInformation.setDocumentName(candidateProfessionalInformation.getDocumentName());
			professionalInformation.setProfessionalDoc(candidateProfessionalInformation.getProfessionalDoc());
	...
	public List<EmployeeEducation> candidateEducationToEmployeeEducationConversion(
		...
		for (CandidateEducation candidateEducation : candidateEducationList) {
			...
			employeeEducation.setDocumentName(candidateEducation.getDocumentName());
			employeeEducation.setEmployeeEducationDoc((candidateEducation.getCandidateEducationDoc()));
			...
			/*Groupg groupg = new Groupg();//comment for future enhancement
			groupg.setGroupId(1l);
	...
	public List<EmployeeNominee> candidateNomineeToEmployeeNominee(List<CandidateNominee> candidateNomineesList) {
		...
		for (CandidateNominee candidateNominee : candidateNomineesList) {
			employeeNomineeList.add(candidateNomineeDtoToDatabaseModel(candidateNominee));
		}
	...
	private EmployeeNominee candidateNomineeDtoToDatabaseModel(CandidateNominee candidateNominee) {
		...
		EmployeeFamily employeeFamily = new EmployeeFamily();
		employeeFamily.setFamilyId(candidateNominee.getCandidateFamily().getFamilyId());;
