...
public class GradeAdaptor implements Adaptor<GradeDTO, Grade> {
	...
	//	Groupg groupg = new Groupg();
		...
		System.out.println("grade..."+gradeDto.getGradesId());
		...
		if(gradeDto.getGradesId()!=null) {
			grade.setActiveStatus(gradeDto.getActiveStatus());
			
		}
		else
			grade.setActiveStatus(EmployeeStatusEnum.ActiveStatus.getEmployeeStatus());
		System.out.println("active Status..."+grade.getActiveStatus());
	...
	public GradeDTO databaseModelToUiDto(Grade grade) {
		...
		gradeDTO.setActiveStatus(grade.getActiveStatus());
