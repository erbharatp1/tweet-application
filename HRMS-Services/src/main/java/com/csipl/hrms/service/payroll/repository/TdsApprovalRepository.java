...
public interface TdsApprovalRepository extends CrudRepository<TransactionApprovedHd, Long>{
	...
	@Query("from TdsTransaction tt  where tt.employee.employeeId=?1 and tt.financialYear.financialYearId=?2")
    public List<TdsTransaction> getTdsTrasactionList(long empId,Long financialYearId);	
