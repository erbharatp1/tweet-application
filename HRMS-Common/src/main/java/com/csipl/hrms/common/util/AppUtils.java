...
public class AppUtils {
	...
	// ="http://10.1.1.100:8081/candidate/#/candidateRegistration?candidateId=";

	public static final String url = "http://localhost:8080/candidate/#/candidateRegistration?candidateId=";
	// letterDeclaration
	public static final String DECLARATION_URL = "http://localhost:8080/hrms#/letterDeclarationTemplate?empLetterId=";

	public static final String ACCEPTANCE_URL = "http://localhost:8080/hrms#/letterDeclarationTemplate?candidateLetterId=";
	 ...
	 * http://localhost:4200/#/letterDeclaration
	 * http://hrms.fabhr.in/hrms#/letterDeclaration
	 * 
	 * int otp = 100000 + generator.nextInt(900000); otpCache.put(key, otp); return
	 * otp; }
	 */
	public List<String> csvReaderFile(File file, Map<String, Attendance> csvEmployees, String processMonth, long userId,
			long companyId, long payRollDays, StringBuffer sb) throws PayRollProcessException {
...
//				seekleave = record.get("Seek Leave");
				...
				if (payDays != null && !payDays.trim().equals("")) {
					BigDecimal dayOfMonth = new BigDecimal(payDays);
					BigDecimal absenset;
					if (!absense.isEmpty() && absense != null) {
						absenset = new BigDecimal(absense);
					} else {
						absenset = BigDecimal.ZERO;
					}
					BigDecimal leave;
					if (!leaves.isEmpty() && leaves != null) {
						leave = new BigDecimal(leaves);
					} else {
						leave = BigDecimal.ZERO;
					}

					int publicholiday;
					if (!publicholidays.isEmpty() && publicholidays != null) {
						publicholiday = Integer.parseInt(publicholidays);
					} else {
						publicholiday = 0;
					}
					int weeklyoff;
					if (!weekoff.isEmpty() && weekoff != null) {
						weeklyoff = Integer.parseInt(weekoff);
					} else {
						weeklyoff = 0;
					}

					BigDecimal presenseDays;

					if (!presense.isEmpty() && presense != null) {
						presenseDays = new BigDecimal(presense);
					} else {
						presenseDays = BigDecimal.ZERO;
					}

					BigDecimal totalDays = absenset.add(leave).add(new BigDecimal(publicholiday))
							.add(new BigDecimal(weeklyoff)).add(presenseDays);

					System.out.println("total days!!!!!!" + totalDays);
					System.out.println("dayOfMonth!!!!!!" + dayOfMonth);
					if (totalDays.compareTo(dayOfMonth) == 1) {
						throw new PayRollProcessException(
								" PayDays should not be greater than PayRoll of " + csvProcessMonth + "  Month ");
					}

					if (new BigDecimal(payRollDays).compareTo(dayOfMonth) != 0) {
						throw new PayRollProcessException(
				...
				if (!presense.isEmpty() && presense != null) {
					...
					attendance.setPresense(BigDecimal.ZERO);
				}

				if (!weekoff.isEmpty() && weekoff != null) {
					attendance.setWeekoff(new BigDecimal(weekoff));
				} else {
					attendance.setWeekoff(BigDecimal.ZERO);
				}

				if (!publicholidays.isEmpty() && publicholidays != null) {
					attendance.setPublicholidays(new BigDecimal(publicholidays));
				} else {
					attendance.setPublicholidays(BigDecimal.ZERO);
				}

				if (!leaves.isEmpty() && leaves != null) {
					attendance.setLeaves(new BigDecimal(leaves));
				} else {
					attendance.setLeaves(BigDecimal.ZERO);
				}
				...
				if (!absense.isEmpty() && absense != null) {
					...
					attendance.setAbsense(BigDecimal.ZERO);
				}
	...
	public static File createXLSFile(MultipartFile file) {
		String csvFileName = "AttandanceXLS";
		File dir = new File(
				File.pathSeparator + HrmsGlobalConstantUtil.APP_BASE_FOLDER + File.pathSeparator + "AttandanceXLS");
		if (!dir.exists()) {
		...
		File fileCsv = null;
		try {
			fileCsv = File.createTempFile(csvFileName, ".xlsx", dir);
			BufferedOutputStream stream = new BufferedOutputStream(new FileOutputStream(fileCsv));
			stream.write(file.getBytes());
			stream.close();
	...
	public static List<String> getEmployeeCode(List<Employee> employeeList) {
		List<String> listEmpCode = new ArrayList<>();
		for (Employee employee : employeeList) {
			listEmpCode.add(employee.getEmployeeCode());
		}
		return listEmpCode;
	...
	public List<String> excelReaderFile(File file, Map<String, OneTimeEarningDeduction> csvOneTimeEarningDeduction,
			String processMonth, long userId, long companyId, long departmentId, long payRollDays, StringBuffer sb)
		...
		try {
			...
			String amount = null, employeeCode = null, employeeName = null, comment = null, srno = null,
					paymentType = null;
			String weekoff = null, publicholidays = null, paidleave = null, casualleave = null, seekleave = null;
			String csvProcessMonth = null;
			...
			for (CSVRecord record : parser) {
				...
				employeeName = record.get("Employee");
				paymentType = record.get("Payment Type").trim();
				amount = record.get("Amount").trim();
				comment = record.get("Comment").trim();
				System.out.println("sNo..." + paymentType + "..." + "Employee Name" + employeeName);
				// absense = record.get("Absent");
				// employeeCode = record.get("Employee Code").trim();
				// sb.append("'").append(employeeCode + "',");
				// employeeName = record.get("Employee Name");
				...
				// csvProcessMonth = record.get("Process month");
				// Attendance attendance = new Attendance();
				OneTimeEarningDeduction oneTimeEarningDeduction = new OneTimeEarningDeduction();
				Company company = new Company();
				company.setCompanyId(companyId);
				// attendance.setUserId(userId);
				// attendance.setCompany(company);
...
//
				 ...
				 * attendance.setDepartment(department); }
				 */
				csvOneTimeEarningDeduction.put(employeeCode, oneTimeEarningDeduction);
				listCsv.add(employeeCode);
			}
