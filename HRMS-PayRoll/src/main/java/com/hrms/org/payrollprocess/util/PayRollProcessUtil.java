...
//			
// PerformanceBonus ( 143 ), 	OverTime(130),
//		Incentive(142)
    ...
    /* else if ( standardEarning == StandardEarningEnum.PerformanceLinkedIncome.getStandardEarning()  ) {
   ...
   } else if ( standardEarning == StandardEarningEnum.PerformanceBonus.getStandardEarning()  ) {
		
		  
		  reportPayOut.setPerformanceBonus( calculatedValue );
		  
   }else if ( standardEarning == StandardEarningEnum.OverTime.getStandardEarning()  ) {
			
			  
			  reportPayOut.setOvertime( calculatedValue );
			  
  }else if ( standardEarning == StandardEarningEnum.Incentive.getStandardEarning()  ) {
		
	
	  reportPayOut.setIncentive( calculatedValue );
	  ...
	  public ReportPayOut fillDeductionValueInReportPayOut( ReportPayOut reportPayOut,
			...
			} else if ( standardDeduction == StandardDeductionEnum.LWF.getStandardDeduction()  ) {
				
				reportPayOut.setLwf( deductionValue );   		
			}else if ( standardDeduction == StandardDeductionEnum.LWF_Employer.getStandardDeduction()  ) {
				
				reportPayOut.setEmployerWelFareAmount(deductionValue);  		
			}
