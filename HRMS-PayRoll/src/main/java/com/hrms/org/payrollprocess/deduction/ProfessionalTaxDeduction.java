...
public class ProfessionalTaxDeduction extends Calculation {
	...
	@Override
	public ArrayList<PayOut> getCalculation(List<PayRollProcessDTO> ptList,
		...
		if ( pt != null ) {
			...
			for (PayRollProcessDTO obj : ptList) {
			...
			for (ProfessionalTaxInfo taxInfo : ptTaxinfoList) {
				...
				if (taxInfo.getCategory().equals(StatusMessage.PTCATEGORY_All_CODE)) {
				...
				} else if (taxInfo.getCategory().equals(payRollProcessHDDTO.getEmployee().getGender())) {
					if (isBetween(grossAmountamount, taxInfo)) {
						perMonthAmount = taxInfo.getTaxAmount();
						ptFlag = true;
						break;
