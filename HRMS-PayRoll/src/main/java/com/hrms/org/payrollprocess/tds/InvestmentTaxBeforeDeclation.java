...
public class InvestmentTaxBeforeDeclation {
	...
	public TdsSummaryChangeDTO calcutateChangeExamptionAndTaxAmount(PayStructureHd payStructureHd,
		...
		System.out.println("daysInMonth================singyyyy a" + daysInMonth);
		...
		if (tdsFullYearFlag) {
			...
			BigDecimal exempPtAmountDays = exempPtAmount.divide(new BigDecimal(daysInMonth), 2, RoundingMode.CEILING);
...
//			exempPtAmountFy=month.multiply(exempPtAmount).add(exempPtAmountDays.multiply(days));
		...
		for (TdsSlabHd tdsSlabHd1 : tdsSlabHdList) {
			...
			/*
			 * else if (age >= 60) { if
			 * (tdsSlabHd1.getTdsSlabMaster().getTdsSlabMasterId()==
			 * StatusMessage.SENIOR_CITIZEN) tdsSlabHd = tdsSlabHd1;
			 * 
