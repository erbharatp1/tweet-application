...
public class EpfCalculation extends Calculation {
	...
	public ArrayList<PayOut> getCalculation(  List<PayRollProcessDTO> epfList , PayRollProcessHDDTO payRollProcessHDDTO , PayrollControl payrollControl) {
		...
		ArrayList<PayOut> listDeduction = new ArrayList<>();
			...
			if (  epf.getMaxBasicLimit().compareTo(baseEarning) ==  1 ||  epf.getMaxBasicLimit().compareTo(baseEarning) == 0) { // EPF amount is less than equal to 150000 ( max limit ) 
				...
				payRollProcessHDDTO.getReportPayOut().setEarnedStdEpfWagesAmount(amount);
				...
				payRollProcessHDDTO.getReportPayOut().setEarnedStdEpfWagesAmount(amount);
				// EPS Wages is Pension Amount
				setPFValueAsActual(payRollProcessHDDTO, amount, listDeduction, epf,pensionAmount);
			//	setPFValue(payRollProcessHDDTO, amount, listDeduction, epf);
				...
				payRollProcessHDDTO.getReportPayOut().setEarnedStdEpfWagesAmount(pensionAmount);
	 ...
	 * @param epf
	 */
	private void setPFValueAsActual(PayRollProcessHDDTO payRollProcessHDDTO, BigDecimal amount, ArrayList<PayOut> listDeduction,
			Epf epf,BigDecimal pensionAmount) {
		BigDecimal employee_contribution_Pf1 = amount.multiply(epf.getEmployeePer()).divide(new BigDecimal(100),
				2, RoundingMode.HALF_UP);
		//21-06-19
		//change employer pf contribution according to Business logic for equal to employee pf contribution 
	//	BigDecimal employer_Contribution_For_PF = amount.multiply(epf.getEmployerPer())
	//			.divide(new BigDecimal(100), 2, RoundingMode.HALF_UP);

		BigDecimal employer_pension_Contribution = pensionAmount.multiply(epf.getEmployerPensionPer())
				.divide(new BigDecimal(100), 2, RoundingMode.HALF_UP);
		
		//
		BigDecimal employer_Contribution_For_PF =  employee_contribution_Pf1
							.subtract(employer_pension_Contribution );
	...
	//	BigDecimal dayWorked = new BigDecimal( daysWorkedStep1 );
        BigDecimal dayWorked = reportPayOut.getPayableDays();		
		BigDecimal pensionAmount1 = maxLimit.divide(payDays, 3, RoundingMode.HALF_UP );
		BigDecimal pensionAmount = pensionAmount1.multiply( dayWorked==null? payDays:dayWorked );
