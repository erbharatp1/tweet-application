...
@EnableJpaRepositories( {"com.csipl.tms.*","com.csipl.common.services.*","com.csipl.hrms.service.*"})
@EnableTransactionManagement

public class TMSPersistenceContext {

	
	private  final String[] ENTITY_PACKAGES = { "com.csipl.tms.model.*","com.csipl.common.model" ,"com.csipl.hrms.model.*"};
	//private static final String[] ENTITY_PACKAGES = { "com.csipl.tms.model.*" };
	 ...
	 * @Bean(destroyMethod="") public DataSource jndiDataSource( Environment env )
	 * throws IllegalArgumentException, NamingException {
	 * 
	 * JndiObjectFactoryBean bean = new JndiObjectFactoryBean(); bean.setJndiName(
	 * env.getRequiredProperty(PROPERTY_NAME_JNDI_NAME) );
	...
	@Primary
	LocalContainerEntityManagerFactoryBean entityManagerFactory(  @Qualifier("dataSource") DataSource dataSource, final Environment env) {
	...
	@Primary
	JpaTransactionManager transactionManager( final @Qualifier("entityManagerFactory") EntityManagerFactory entityManagerFactory) {
