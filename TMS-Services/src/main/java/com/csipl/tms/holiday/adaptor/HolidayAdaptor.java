...
public class HolidayAdaptor implements Adaptor<HolidayDTO, TMSHolidays> {
	...
	public List<TMSHolidays> holidayObjectlistToDatabaseModelList(List<Object[]> holidayObjList) {
		List<TMSHolidays> holidayList = new ArrayList<TMSHolidays>();
		for (Object[] tmsHolidaysObj : holidayObjList) {
			TMSHolidays holiday = new TMSHolidays();
			String holiDayName = tmsHolidaysObj[1] != null ? (String) tmsHolidaysObj[1] : null;
			Long companyId = tmsHolidaysObj[2] != null ? Long.parseLong(tmsHolidaysObj[2].toString()) : null;
			Date fromDate = tmsHolidaysObj[3] != null ? (Date) (tmsHolidaysObj[3]) : null;
			Date toDate = tmsHolidaysObj[4] != null ? (Date) (tmsHolidaysObj[4]) : null;
			Long day = tmsHolidaysObj[5] != null ? Long.parseLong(tmsHolidaysObj[5].toString()) : null;
