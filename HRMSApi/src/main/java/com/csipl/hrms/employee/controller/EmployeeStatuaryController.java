...
public class EmployeeStatuaryController {
	...
	public List<EmployeeStatuaryDTO> employeeStatuarys(@PathVariable("empId") String empId,
			@RequestBody List<EmployeeStatuaryDTO> employeeStatuaryDtoList, HttpServletRequest req) throws ErrorHandling{
		...
		employeeStatuary.getStatuaryId() != null ? false : true;*/
			 System.out.println("idd..."+employeeStatuary.getEmployeeFamily().getFamilyId());
	...
	public @ResponseBody List<EmployeeStatuaryDTO> findAllEmployeeStatuarys(@PathVariable("empId") String empId,
		...
		logger.info("findAllEmployeeStatuarys... empId :" + empId);
		Long employeeId = Long.parseLong(empId);
	 ...
	 * to get List of EmployeeStatuary from database based on employeeId
	 */
	@RequestMapping(value="/statuaryForNominee/{empId}", method = RequestMethod.GET)
	public @ResponseBody List<EmployeeStatuaryDTO> findAllEmployeeStatuarysWithoutUAN(@PathVariable("empId") String empId,
			HttpServletRequest req) {
		logger.info("findAllEmployeeStatuarys... empId :" + empId);
		Long employeeId = Long.parseLong(empId);
		return employeeStatuaryAdaptor
				.databaseModelToUiDtoList(employeeStatuaryService.findAllEmployeeStatuaryWithoutUAN(employeeId));
	}
	/**
	 * to get List of EmployeeStatuary from database based on employeeId
	 */
	@RequestMapping(value="/statuaryNominee/{empId}", method = RequestMethod.GET)
	public @ResponseBody List<EmployeeStatuaryDTO> findEmployeeStatuaryNominee(@PathVariable("empId") String empId,
			HttpServletRequest req) {

		Long employeeId = Long.parseLong(empId);
		return employeeStatuaryAdaptor
				.databaseModelToUiDtoList(employeeStatuaryService.findEmployeeStatuaryNominee(employeeId));
	}
