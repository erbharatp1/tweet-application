...
public class CandidateIdAddressController {
	...
	public List<CandidateIdProofDTO> saveCandidateIdProof(@PathVariable("canid") String canId, HttpServletRequest req)
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			if(!(("").equals(jsonObj.getString("toDate")))&&jsonObj.getString("toDate") != null && !(("").equals(jsonObj.getString("dateFrom")))&&jsonObj.getString("dateFrom") != null ) {
				...
				candidateIdDto.setDateFrom(dateformat.parse(dateFrom));	
			}
		    else if ( !(("").equals(jsonObj.getString("toDate")))&&jsonObj.getString("toDate") != null) {
				String dateTo = jsonObj.getString("toDate");
				candidateIdDto.setDateTo(dateformat.parse(dateTo));	
				
			}else  if(!(("").equals(jsonObj.getString("dateFrom")))&&jsonObj.getString("dateFrom") != null) {
				String dateFrom = jsonObj.getString("dateFrom");
				candidateIdDto.setDateFrom(dateformat.parse(dateFrom));	
			} 	
...
//			}
	...
	@RequestMapping(value = "/canIdAdressProf/{candidateId}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody CandidateIdProofDTO get(@PathVariable("candidateId") Long candidateId) {
		System.out.println("candidateIdAddress service is calling : " + candidateId);
		CandidateIdProof candidateIdProof = candidateIdAddressService.getCandidateIdproof(candidateId);
	
		CandidateIdProofDTO candidateIdProofDTO = candidateIdAddressAdaptor.databaseModelToUiDto(candidateIdProof);
