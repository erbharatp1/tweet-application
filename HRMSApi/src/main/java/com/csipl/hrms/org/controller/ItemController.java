...
public class ItemController {
	...
	public @ResponseBody ItemDTO saveItems(@RequestBody ItemDTO itemDto , HttpServletRequest req) {
		logger.info("saveItems is calling");
		 Item  item = itemAdaptor.uiDtoToDatabaseModel(itemDto);
		return itemAdaptor.databaseModelToUiDto(itemService.save(item));
	}
	...
	*/
	@RequestMapping(value="deleteItems/{itemId}", method = RequestMethod.GET)
	public @ResponseBody int deleteItem(@PathVariable("itemId") String itemId,
			HttpServletRequest req) {
		logger.info("deleteTicketType is calling :" + "ticketTypeId  ==" + itemId);
		return  itemService.delete(Long.valueOf(itemId));
	}
}
