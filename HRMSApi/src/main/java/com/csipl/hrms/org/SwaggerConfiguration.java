 ...
 * @Configuration
 * 
 * @EnableSwagger2 //@ComponentScan(basePackages = "com.csipl.hrms")
 * 
 * @EnableAutoConfiguration public class SwaggerConfiguration {
 * 
 * 
 * 
 * 
 * @Bean public Docket hrmsApi() { return new
 * Docket(DocumentationType.SWAGGER_2) .select()
 * .apis(RequestHandlerSelectors.basePackage("com.csipl.hrms"))
 * .paths(PathSelectors.any()) // .paths(regex("/hrmsApi.*"))
 * .apis(RequestHandlerSelectors.any()) // .paths( regex("/hrmsApi.*") )
 * .build().apiInfo(metaData()); }
 * 
 * private ApiInfo metaData() {
 * 
 * return new ApiInfoBuilder() .title("Computronics HRMS API")
 * .description("Computronics HRMS API  REST API for Pay Roll System")
 * .termsOfServiceUrl("") .contact(new
 * Contact("Shailendra Kumar","","shailendra@computronics.in"))
