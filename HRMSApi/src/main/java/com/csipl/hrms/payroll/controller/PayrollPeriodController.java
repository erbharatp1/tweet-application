...
public class PayrollPeriodController {
	...
	public @ResponseBody ErrorHandling save(@RequestBody FinancialYearDTO financialYearDto)
			throws PayRollProcessException {
		...
		FinancialYear financialYear1 = payrollPeriodService.findFinancialYear(checkFinancialYear,
				financialYearDto.getCompanyId());
		if (financialYear1 == null || financialYearDto.getFinancialYearId() != null) {
			...
			errorHandling=	payrollPeriodService.save(financialYear);
		} else {
	...
	@RequestMapping(path = "/financialYearList/{companyId}", method = RequestMethod.GET)
	public @ResponseBody List<FinancialYearDTO> getFinancialYearList(@PathVariable("companyId") String companyId)
			throws ErrorHandling, PayRollProcessException {
		logger.info("getFinancialYearList : companyId" + companyId);
		Long longCompanyId = Long.parseLong(companyId);

		List<FinancialYear> financialYearList = payrollPeriodService.getFinancialYearList(longCompanyId);
		if (financialYearList != null) {
			return payrollAdaptor.databaseModelToUiDtoList(financialYearList);

		} else
			throw new ErrorHandling("FinancialYear are not available in company");

	}

	@RequestMapping(value = "/financialYearStatusUpdate", method = RequestMethod.POST)
	public void financialYearStatusUpdate(@RequestBody FinancialYearDTO financialYearDto) {
		logger.info("FinancialYearDTO is calling :  " + financialYearDto);
		...
		company.setCompanyId(financialYearDto.getCompanyId());
		financialYear.setCompany(company);
		payrollPeriodService.updateById(financialYear);

	}



	@RequestMapping(value = "/financialMonth/{companyId}", method = RequestMethod.GET)
	public @ResponseBody FinancialMonthDTO getDepartment(@PathVariable("companyId") String companyId) {
		logger.info("getFinancialMonth is calling : financialMonth =" + companyId);
		Long companyIds = Long.parseLong(companyId);
		return payrollAdaptor.databaseModelToUiMonthDto(payrollPeriodService.getFinancialMonth(companyIds));
	}

	@RequestMapping(path = "/payrollControl/updatePayrollLock", method = RequestMethod.POST)
	public void updatePayrollLock(@RequestBody PayrollControlDTO payrollControlDTO) {
		payrollControlService.updatePayrollLock(payrollControlDTO.getControlId(), payrollControlDTO.getIspayrollLocked());
		
	}
	
	@RequestMapping(value = "/payrollControl/financialYearId/{financialYearId}", method = RequestMethod.GET)
	public @ResponseBody List<PayrollControlDTO> payrollControlByfinancialYearId(@PathVariable("financialYearId") Long financialYearId) {
		List<PayrollControl> PayrollControlList = payrollControlService.findPayrollControlByFYId(financialYearId);
		return payrollControlAdaptor.databaseModelToUiDtoList(PayrollControlList);
	}
	
	@RequestMapping(value = "/allProcessMonth/{companyId}", method = RequestMethod.GET)
	public @ResponseBody List<PayrollControlDTO> payrollControlByCompanyId(@PathVariable("companyId") Long companyId) {
		List<PayrollControl> PayrollControlList = payrollControlService.payrollControlByCompanyId(companyId);
		return payrollControlAdaptor.databaseModelToUiDtoList(PayrollControlList);
	}
}
