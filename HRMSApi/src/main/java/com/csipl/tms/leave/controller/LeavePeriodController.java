...
public class LeavePeriodController {
	...
	public TMSLeavePeriodDTO leavePeriod(@PathVariable("companyId") Long companyId) {
		logger.info("leavePeriodList is calling : " + " : companyId " + companyId);
		//Long companyID = Long.parseLong(companyId);
		TMSLeavePeriod leavePeriod = leavePeriodService.leavePeriod(companyId);
		TMSLeavePeriodDTO leavePeriodDto = leavePeriodAdaptor.databaseModelToUiDto(leavePeriod);
		//logger.info("leavePeriodList is end  :" + "shiftDtoList" + leavePeriodDtoList);
		return leavePeriodDto;
	}
	
	@RequestMapping(value="periodSession/{companyId}",method = RequestMethod.GET)
	public TMSLeavePeriodDTO leavePeriodSession(@PathVariable("companyId") Long companyId) throws ErrorHandling {
		logger.info("leavePeriodSession is calling : " + " : companyId " + companyId);
		//Long companyID = Long.parseLong(companyId);
		...
		List<TMSLeavePeriod> leavePeriodList = leavePeriodService.findLeavePeriodByProcessMonthWithACflag(companyId , psMonthLocalDate.toString());
		
		if(leavePeriodList == null) {
			throw new ErrorHandling("Leave Session Expired!");
		}else if(leavePeriodList.size() == 0){
			throw new ErrorHandling("Leave Session Expired!");
		}
		
		return new TMSLeavePeriodDTO();
	}
}
