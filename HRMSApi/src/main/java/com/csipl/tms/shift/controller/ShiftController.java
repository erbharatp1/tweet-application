...
public class ShiftController {
	...
	@RequestMapping(value = "/companyshift/{companyId}", method = RequestMethod.GET)
	public ShiftDTO shiftList(@PathVariable("companyId") Long companyId) {
	...
	public List<ShiftDTO> shiftDtoList(@PathVariable("companyId") Long companyId) {
		logger.info("shiftList is calling : " + " : companyId " + companyId);
		// Long companyID = Long.parseLong(companyId);
		ShiftDTO shiftDto = new ShiftDTO();
		List<Shift> shiftList = shiftService.findAllShift(companyId);
	...
	public ShiftDTO findshift(@PathVariable(value = "shiftId") Long shiftId) {
	...
	/*
	 * get shift time by employeeId
	 */
	@RequestMapping(path = "/shiftDuration/{employeeId}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public ShiftDurationDTO findDuration(@PathVariable(value = "employeeId") Long employeeId) {
		List<Object[]> obj = shiftService.findShiftDuration(employeeId);

		return shiftAdaptor.objectarrayToDto(obj);
	}

	@RequestMapping(value = "fetchAllShift/{companyId}", method = RequestMethod.GET)
	public List<ShiftDTO> fetchAllShift(@PathVariable("companyId") Long companyId) {
		logger.info("fetchAllShift is calling : " + " : companyId " + companyId);
		// Long companyID = Long.parseLong(companyId);
		ShiftDTO shiftDto = new ShiftDTO();
		List<Shift> shiftList = shiftService.fetchAllShift(companyId);
		List<ShiftDTO> shiftDtoList = shiftAdaptor.databaseModelToUiDtoList(shiftList);

		logger.info("fetchAllShift is end  :" + "shiftDtoList" + shiftDtoList);
		return shiftDtoList;
	}
}